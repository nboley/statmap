...
{
    *reads = malloc(sizeof(rawread_array));
    (*reads)->size = 0;
    (*reads)->allocated_size = READ_ARRAY_GROWTH_FACTOR;
    (*reads)->reads = malloc(sizeof(rawread)*READ_ARRAY_GROWTH_FACTOR);
    return;
}
...
add_rawread_to_array( rawread_array* reads,
                      rawread* r            )
{
    /* allocate more memory for the reads structure if we are out */
    reads->size++;
    if( reads->size == reads->allocated_size )
    {
        reads->allocated_size += READ_ARRAY_GROWTH_FACTOR;
        reads->reads = realloc( reads->reads, sizeof(rawread)*(reads->allocated_size) );
    }
    
    memcpy( reads->reads + reads->size - 1, r, sizeof( rawread ) );
                  ...
                  int (* filter_rawread )( rawread* )    )
{
    ...
    /* BUG - overflow error */
    char unmappable_fname[500];
    strcpy( unmappable_fname, input_filename );
    strcat( unmappable_fname, ".unmappable" );
    FILE *unmappable_fp = fopen( unmappable_fname, "w" );
    ...
    /* initialize the reads array */
    init_rawread_array( &reads );
    ...
    while( !feof(input_file) ) {
        ...
        if( num_reads % 10000 == 0 )
            fprintf( stderr, "Processed %i reads\n", num_reads );
        
        rawread* r;
        
        /* populate the read - this is file type specific */
        error = populate_read_from_fp( input_file, &r );
        assert( error == 0 );
        
        
        /* Translate the 5' read */
        if( NULL == translate_seq(r->char_seq, r->length, &translated_read) ) {
            //fprintf(stderr, "Could not translate: %s\n", read);
            continue;
        }

        add_rawread_to_array( reads,
                              readname,
                              read_end,
                              0,
                              translated_read,
        ...
        free( translated_read );

        /* map the read if it came from the top strand */
        
        if( 5 == rev_complement_read( read, mut_read, read_len ) ) {
            //fprintf(stderr, "Could not reverse: %s\n", read);
            continue;
        }
        
        if( NULL == translate_seq(mut_read, read_len, &translated_read ) ) {
            //fprintf(stderr, "Could not translate: %s\n", mut_read);
            continue;
        } 
