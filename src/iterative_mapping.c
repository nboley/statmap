...
)
{
    ...
    update_trace_segments_from_uniform_kernel(
        trace_segments, BIN_SIZE*scale_factor, start/BIN_SIZE, stop/BIN_SIZE+1);
                                 ...
                                 mapped_read_t* r  )
{
    ...
    for( i = 0; i < rd_index->num_mappings; i++ ) {
        ...
        SIGNED_LOC pos1 = get_start_from_mapped_read_location(loc);
        SIGNED_LOC pos2 = get_stop_from_mapped_read_location(loc);

        local_read_sum[i] = accumulate_from_traces(
            traces, contig, pos1/BIN_SIZE, pos2/BIN_SIZE + 1);
    ...
    for( i = 0; i < rd_index->num_mappings; i++ )
    {
        ...
        window_density += accumulate_from_traces(
            traces, chr_index, start/BIN_SIZE, stop/BIN_SIZE + 1);
